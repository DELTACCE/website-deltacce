          ...
          <div className="hidden md:flex items-center gap-4 text-sm relative">
            {visibleLinks.map((link) => (
              <NavLink
                ...
                to={link.path}
                className={({ isActive }) =>
                  `relative py-1.5 px-3 transition-colors duration-300 hover:text-signal tracking-wide ${
                    isActive ? 'text-signal font-semibold' : 'text-indigo font-normal'
                  }`
                }
              >
                {({ isActive }) => (
            ...
            {hasManyLinks && (
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-1 py-1.5 px-3 text-indigo hover:text-signal font-semibold transition-colors duration-300 focus:outline-none"
                >
                  <span>More</span>
                  <motion.span
                  ...
                  {dropdownOpen && (
                    <>
                      ...
                      <motion.div
                        ...
                        className="absolute right-0 mt-2 w-48 bg-paper border border-indigo/10 rounded-2xl shadow-xl py-2 z-20 overflow-hidden"
                      >
                        {dropdownLinks.map((link) => (
                          <NavLink
                            ...
                            className={({ isActive }) =>
                              `block px-4 py-2.5 text-xs tracking-wide transition-colors duration-200 hover:bg-indigo/5 ${
                                isActive ? 'text-signal font-semibold' : 'text-indigo font-normal'
                              }`
                            }
                          >
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            <div className="flex justify-between items-center pb-6 border-b border-indigo/10">
              <Link to="/" onClick={() => setIsOpen(false)} className="text-xl font-bold text-indigo flex items-center gap-2 select-none">
                <span className="text-2xl text-signal font-extrabold">δ</span>
                <span className="tracking-widest text-indigo">DELTA</span>
              </Link>
              <button
                onClick={() => setIsOpen(false)}
                className="text-indigo hover:text-signal p-2 focus:outline-none transition-colors"
            ...
            <div className="flex-grow flex flex-col justify-between py-12">
                ...
                {navLinks.map((link, idx) => (
                  ...
                  >
                    <NavLink
                      ...
                      onClick={() => setIsOpen(false)}
                      className={({ isActive }) =>
                        `text-4xl font-heading tracking-wide uppercase transition-colors duration-300 block ${
                          isActive ? 'text-signal font-extrabold' : 'text-indigo hover:text-signal'
                        }`
                      }
                    >
                      {link.name}
              ...
              {/* Tagline & Info Box */}
              <div className="mt-8 border border-indigo/10 bg-paper/50 p-6 rounded-2xl relative shadow-md">
                ...
                <h2 className="font-heading text-xl font-extrabold text-indigo uppercase mb-3">
                ...
                <div className="flex flex-col gap-3">
                  <Link
                    to="/committee"
                    onClick={() => setIsOpen(false)}
                    className="inline-flex items-center justify-center font-heading text-xs font-bold tracking-wider uppercase px-6 py-3.5 border-2 bg-signal border-signal text-paper rounded-full hover:bg-transparent hover:text-signal transition-all duration-300 shadow-sm"
                  >
                    MEET THE TEAM
                  </Link>
                  <Link
                    to="/about"
                    onClick={() => setIsOpen(false)}
                    className="inline-flex items-center justify-center font-heading text-xs font-bold tracking-wider uppercase px-6 py-3.5 border-2 border-indigo text-indigo rounded-full hover:bg-indigo hover:text-paper transition-all duration-300"
                  >
                    EXPLORE DELTA
                  </Link>
                </div>
